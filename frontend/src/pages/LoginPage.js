...
import { useSignIn, useClerk } from '@clerk/clerk-react';   // Clerk Imports

function LoginPage() {
  const { isLoaded, signIn } = useSignIn();  //useSignIn Hook
  const { signOut } = useClerk();            //useClerk Hook for logout/general tasks
  ...
  const handleMainSubmit = async (e) => {
    ...
    if (role === "admin") {
      ...
      if (!itRegex.test(formData.itNumber)) {  //validation 
        setLoading(false);
  ...
  const handleSocialLogin = async (strategy) => {
    if (!isLoaded) {
      setErrorStr("Clerk is still loading. Please try again in a moment.");
      return;
    }
    try {
      setLoading(true);
      await signIn.authenticateWithRedirect({    //OAuth Strategy via useSignIn
        strategy: strategy,
        redirectUrl: "/dashboard",    
        redirectUrlComplete: "/dashboard"
      });
    } catch (err) {
      console.error("Clerk Auth Error Details:", err);
      // More detailed error reporting
      if (err.errors && err.errors.length > 0) {
        setErrorStr(err.errors[0].longMessage);
      } else if (err.message) {
        setErrorStr(`Clerk Error: ${err.message}`);
      } else {
        setErrorStr("Connection failed to Clerk services. Please check your internet or Clerk configuration.");
      }
      setLoading(false);
    }
  };
            ...
            <form onSubmit={handleMainSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-3 py-4 bg-slate-900 text-white rounded-2xl font-bold hover:bg-slate-800 transition-all active:scale-[0.98] disabled:opacity-50"
                >
                  <div className="w-6 h-6 bg-white rounded-lg flex items-center justify-center">
                     <svg className="w-4 h-4 text-slate-900" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M21.35,11.1H12.18V13.83H18.69C18.36,17.64,15.19,19.27,12.19,19.27C9.03,19.27,6.48,16.85,6.48,12.05S9.03,4.83,12.19,4.83C14.31,4.83,16.21,5.84,17.27,7.34L19.27,5.34C17.61,3.42,15.02,2.1,12.19,2.1A9.95,9.95,0,0,0,2.24,12.05A9.95,9.95,0,0,0,12.19,22A9.95,9.95,0,0,0,22.14,12.05C22.14,11.73,22.11,11.42,21.35,11.1Z" />
                     </svg>
                  </div>
                  <span className="text-xs uppercase tracking-widest">Continue with Clerk</span>
                </button>
              </div>
            </form>
